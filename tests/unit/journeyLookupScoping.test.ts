...
//
// Runnable via:
//   npx tsx tests/unit/journeyLookupScoping.test.ts
//
// PURPOSE
...
import { journeyLookupFilter } from "../../server/services/engagement/journeyLookupRules";
...
if (failures.length > 0) {
  console.error("journeyLookupScoping.test.ts: failures:");
  for (const f of failures) console.error(`- ${f}`);
  process.exit(1);
}
console.log("journeyLookupScoping.test.ts: all tests passed");
