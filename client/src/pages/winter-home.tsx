...
// ─── Configurable glass edge panels ──────────────────────────────────────────
// Both screen edges host an expandable frosted-glass panel (same glass as the
// dock). Each panel's content is configurable and persisted per side; content
// renders as an embedded app surface. The left panel defaults to the ACS/PCS
// team portal.
type EdgePanelContent = { id: string; label: string; href: string };

const EDGE_PANEL_CONTENT: EdgePanelContent[] = [
  { id: "acs", label: "ACS Portal", href: "/ancillary-care-specialist-portal" },
  { id: "pcs", label: "PCS Portal", href: "/patient-care-specialist-portal" },
  { id: "schedule", label: "Schedule", href: "/schedule" },
  { id: "patient-directory", label: "Patient Database", href: "/patient-directory" },
  { id: "plexus-tasks", label: "Task Brain", href: "/plexus-tasks" },
  { id: "billing", label: "Billing", href: "/billing" },
];

const edgePanelKey = (side: "left" | "right") => `winterHome.edgePanel.${side}`;

function WinterEdgePanel({ side, defaultContentId }: { side: "left" | "right"; defaultContentId: string }) {
  const [open, setOpen] = useState(false);
  const [contentId, setContentId] = useState<string>(() => {
    try {
      const saved = localStorage.getItem(edgePanelKey(side));
      if (saved && EDGE_PANEL_CONTENT.some((c) => c.id === saved)) return saved;
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(edgePanelKey(side), contentId);
    } catch {
      /* ignore */
    }
  }, [side, contentId]);

  const content = EDGE_PANEL_CONTENT.find((c) => c.id === contentId) ?? EDGE_PANEL_CONTENT[0];
  const isLeft = side === "left";
  const Chevron = open ? (isLeft ? ChevronLeft : ChevronRight) : isLeft ? ChevronRight : ChevronLeft;

  // Lighter bluish-purple button token used across the panel chrome.
  const periwinkleBtn =
    "bg-indigo-300/40 hover:bg-indigo-300/60 text-indigo-50 border border-indigo-200/40 transition-colors";

  return (
    <>
      {/* Collapsed edge handle — glass pill, mid-edge, fades like the dock. */}
      {!open && (
        <button
          type="button"
          onClick={() => setOpen(true)}
          className={`absolute top-1/2 -translate-y-1/2 ${isLeft ? "left-0 rounded-r-xl" : "right-0 rounded-l-xl"} z-[45] flex h-24 w-6 items-center justify-center bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl text-indigo-100 opacity-40 hover:opacity-100 hover:bg-indigo-300/25 transition-all`}
          aria-label={`Open ${isLeft ? "left" : "right"} panel`}
          data-testid={`edge-panel-handle-${side}`}
        >
          <Chevron className="w-4 h-4" strokeWidth={2.4} />
        </button>
      )}

      {/* Expanded panel — overlays the desktop under the banner; never pushes layout. */}
      <div
        className={`absolute top-12 bottom-0 ${isLeft ? "left-0 border-r" : "right-0 border-l"} z-[45] flex w-[620px] max-w-[94vw] flex-col overflow-hidden bg-white/10 backdrop-blur-xl border-white/20 shadow-[0_30px_90px_rgba(15,23,42,0.45)] transition-transform duration-300 ease-out ${
      ...
      >
        <div className="flex h-11 shrink-0 items-center gap-2 border-b border-white/15 bg-white/10 px-3">
          {(contentId === "acs" || contentId === "pcs") ? (
            <div className="flex items-center gap-1" data-testid={`edge-panel-portal-toggle-${side}`}>
              {(["acs", "pcs"] as const).map((id) => (
                <button
                  key={id}
                  type="button"
                  onClick={() => setContentId(id)}
                  className={`h-7 rounded-md px-3 text-[11px] font-semibold uppercase tracking-wider ${
                    contentId === id
                      ? "bg-indigo-300/60 text-white border border-indigo-200/60 shadow-sm"
                      : periwinkleBtn
                  }`}
                  data-testid={`edge-panel-tab-${id}-${side}`}
                >
                  {id.toUpperCase()}
                </button>
              ))}
            ...
            <span className="text-[12px] font-semibold tracking-tight text-white/90">{content.label}</span>
          )}
          <div className="ml-auto flex items-center gap-1.5">
            <select
              value={contentId}
              onChange={(e) => setContentId(e.target.value)}
              className="h-7 rounded-md border border-indigo-200/40 bg-indigo-300/30 px-2 text-[11px] font-medium text-indigo-50 focus:outline-none focus:ring-1 focus:ring-indigo-200/70 [&>option]:text-slate-900"
              aria-label="Panel content"
              data-testid={`edge-panel-content-select-${side}`}
            >
              {EDGE_PANEL_CONTENT.map((c) => (
            ...
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-md ${periwinkleBtn}`}
        ...
        <div className="relative min-h-0 flex-1">
          {open && (
            <iframe
              src={`${content.href}?embed=1`}
              title={content.label}
              className="absolute inset-0 h-full w-full border-0 bg-white/85"
              data-testid={`edge-panel-iframe-${side}`}
            />
          )}
        </div>
      </div>
...
export default function WinterHomePage({ user }: { user?: AuthUser }) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden font-sans select-none">
      ...
      {/* Configurable glass edge panels — overlay the desktop under the banner. */}
      <WinterEdgePanel side="left" defaultContentId="acs" />
      <WinterEdgePanel side="right" defaultContentId="schedule" />
