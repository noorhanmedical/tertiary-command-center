  ...
  return (
    <div className="flex flex-col h-full relative z-10 bg-finance-bg overflow-hidden">
      ...
      {railCollapsed && hasSelection ? (
        <div
          className="hidden lg:flex flex-col items-center shrink-0 pt-3 border-r border-[#D3DCE8] bg-[#E4EAF3] dark:bg-card/30"
          style={{ width: "40px" }}
      ...
      <aside
        className={`${hasSelection ? "hidden lg:flex" : "flex"} flex-col w-full lg:w-[280px] xl:w-[300px] shrink-0 border-r border-[#D3DCE8] bg-[#E4EAF3] dark:bg-card/30`}
              ...
              {mergedRoster.groups.map((group) => {
                ...
                return (
                  <section key={group.clinic} data-testid={`section-clinic-${group.clinic.replace(/\s+/g, "-")}`}>
                    <div className="flex items-center gap-2 px-2 mb-1.5 sticky top-0 bg-[#E4EAF3]/90 dark:bg-card/80 backdrop-blur py-1 z-10">
                      ...
                      {group.patients.map((p) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left rounded-lg px-2.5 py-2 transition-colors flex items-start gap-2.5 border-l-2 ${selected ? "bg-[#DCE6F5] dark:bg-indigo-900/30 border-[#3169E8]" : "border-transparent hover:bg-[#EDF2F9] dark:hover:bg-muted/50"}`}
                            data-testid={`card-patient-${p.encodedKey}`}
                          >
                            <div className="w-8 h-8 rounded-full bg-[#DCE6F5] dark:bg-slate-800 flex items-center justify-center shrink-0 text-[11px] font-semibold text-[#263B63] dark:text-slate-300" data-testid={`avatar-patient-${p.encodedKey}`}>
                              {initials(p.name)}
                            </div>
                            <div className="min-w-0 flex-1">
