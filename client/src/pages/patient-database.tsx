  ...
  return (
    <div className="flex flex-col h-full relative z-10 bg-finance-bg overflow-hidden">
      ...
      <div className="shrink-0 px-6 pt-6 pb-4 flex flex-wrap items-center justify-between gap-3 border-b border-border/50">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            data-testid="button-import-test-history"
          >
            <Upload className="w-3.5 h-3.5" />Import History
          </Button>
          <Button
            size="sm"
            variant="outline"
      ...
      {railCollapsed && hasSelection ? (
        <div
          className="hidden lg:flex flex-col items-center shrink-0 pt-3 border-r border-[#33405A] bg-[#20293B]"
          style={{ width: "40px" }}
      ...
      <aside
        className={`ehr-roster-dark ${hasSelection ? "hidden lg:flex" : "flex"} flex-col w-full lg:w-[280px] xl:w-[300px] shrink-0 border-r border-[#33405A] bg-[#20293B]`}
        data-testid="patient-directory-rail"
      >
        {/* Dark-panel text/control overrides scoped to this rail only, so the
            deep winter-slate list reads with light text without rewriting every
            nested utility class. */}
        <style>{`
          .ehr-roster-dark, .ehr-roster-dark h2 { color: #E7ECF5; }
          .ehr-roster-dark .text-muted-foreground { color: #93A0B8 !important; }
          .ehr-roster-dark .text-slate-800, .ehr-roster-dark .text-slate-700,
          .ehr-roster-dark .text-slate-600, .ehr-roster-dark .text-foreground { color: #E7ECF5 !important; }
          .ehr-roster-dark input { color: #E7ECF5; }
          .ehr-roster-dark input::placeholder { color: #7E8CA6; }
          /* Selected patient row is an elevated LIGHT card — restore dark text
             inside it so it reads on white. */
          .ehr-roster-dark .bg-white .text-muted-foreground { color: #64748B !important; }
          .ehr-roster-dark .bg-white .text-slate-800,
          .ehr-roster-dark .bg-white .text-slate-700,
          .ehr-roster-dark .bg-white .text-slate-600,
              ...
              {mergedRoster.groups.map((group) => {
                ...
                return (
                  <section key={group.clinic} data-testid={`section-clinic-${group.clinic.replace(/\s+/g, "-")}`}>
                    <div className="flex items-center gap-2 px-2 mb-1.5 sticky top-0 bg-[#20293B]/95 backdrop-blur py-1 z-10">
                      <Building2 className="w-3.5 h-3.5 text-[#93A0B8]" />
                      <h2 className="text-xs font-semibold text-[#C4D0E6] truncate">{group.clinic}</h2>
                      <Badge variant="outline" className="text-[10px] border-[#3A4966] text-[#93A0B8]" data-testid={`badge-clinic-count-${group.clinic.replace(/\s+/g, "-")}`}>
