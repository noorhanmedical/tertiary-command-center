...
const railItems = ["PX", "EC", "PD", "PC", "AC", "BI"] as const;

export default function EngagementCenterPage() {
  return (
    <div className="relative left-1/2 flex h-full w-screen -translate-x-1/2 bg-[#EEF1F6] text-[#101115]">
      <aside className="hidden w-[76px] shrink-0 flex-col items-center gap-4 border-r border-slate-800 bg-[#101115] px-0 py-5 text-white lg:flex">
        {railItems.map((item) => (
          <div
            key={item}
            className={`grid h-11 w-11 place-items-center text-[11px] font-semibold tracking-[0.04em] ${
              item === "EC" ? "bg-white text-[#101115]" : "text-slate-400"
            }`}
          >
            {item}
          </div>
              ...
              <div className="relative z-10 grid gap-8 xl:grid-cols-[minmax(0,1fr)_420px]">
                <div>
                  ...
                  <div className="mt-8 flex flex-wrap gap-2">
                    <button type="button" className="border border-white bg-white px-4 py-2 text-xs font-light uppercase tracking-[0.08em] text-[#101115]">
                      Assign selected
                    </button>
                    <button type="button" className="border border-white/20 bg-white/10 px-4 py-2 text-xs font-light uppercase tracking-[0.08em] text-white">
                      Unassigned
                    </button>
                    <button type="button" className="border border-white/20 bg-white/10 px-4 py-2 text-xs font-light uppercase tracking-[0.08em] text-white">
                      Interested
