// Admin Settings → API Integration Station — vendor-neutral EMR
// integration framework.
//
// PHASE 1 SCOPE: this page renders the admin UI + scope/mapping/routing
// model + audit/error shells for third-party EMR integrations.
// eClinicalWorks (FHIR R4) is the first available vendor adapter; Epic,
// Athena, Oracle Health/Cerner, NextGen, Veradigm, Meditech, and other
// FHIR R4 EMRs are catalogued as future vendor adapters.
//
...
//
// See `docs/architecture/api-integration-station-emr-foundation.md` for
// the vendor-neutral architecture plan and
// `docs/architecture/ecw-fhir-r4-integration-foundation.md` for the
// eCW-specific Phase 2 backend plan.
...
import {
  INTEGRATION_VENDOR_ADAPTERS,
} from "@/lib/api-integrations/integrationVendors";
import {
  INTEGRATION_DOWNSTREAM_DESTINATIONS,
  INTEGRATION_PIPELINE_STAGES,
} from "@/lib/api-integrations/integrationRouting";
import {
  INTEGRATION_PHASE_1_SUMMARY,
  INTEGRATION_SECURITY_RULES,
  WRITEBACK_FUTURE_CANDIDATES,
  WRITEBACK_READINESS_REQUIREMENTS,
} from "@/lib/api-integrations/integrationSecurity";
...
const SECTION_NAV: SectionNavGroup[] = [
  {
    label: "Overview",
    items: [
      { id: "overview", label: "API Integration Station", Icon: Layers },
      { id: "vendors", label: "Vendor Adapters", Icon: PackageOpen },
    ],
  },
  {
    label: "eClinicalWorks · vendor adapter",
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "writeback", label: "Writeback Readiness", Icon: ShieldX },
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            {active === "vendors" && <VendorsSection setActive={setActive} />}
            {active === "connection" && (
            ...
            {active === "writeback" && <WritebackReadinessSection />}
...
function OverviewSection({ setActive }: { setActive: (id: SectionId) => void }) {
  const availableVendors = INTEGRATION_VENDOR_ADAPTERS.filter((v) => v.status === "Available").length;
  const futureVendors = INTEGRATION_VENDOR_ADAPTERS.filter((v) => v.status === "Future").length;

  return (
    <SectionShell
      title="API Integration Station"
      subtitle="Central, vendor-neutral admin console for third-party EMR + FHIR integrations. Phase 1 ships the UI + mapping + routing model; Phase 2 ships the sync worker, credential vault, and normalization pipeline."
      status={<Badge variant="secondary" className="text-[10px]">Foundation</Badge>}
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <IntegrationKpiCard label="Available vendor adapters" value={availableVendors} hint="eClinicalWorks ships first." />
        <IntegrationKpiCard label="Future vendor adapters" value={futureVendors} hint="Catalogued in the Vendor Adapters section; disabled in the UI." />
        <IntegrationKpiCard label="eCW FHIR R4 scopes" value={ECW_SCOPE_DEFINITIONS.length} hint="All 18 read scopes represented." />
        <IntegrationKpiCard label="Downstream modules" value={INTEGRATION_DOWNSTREAM_DESTINATIONS.length} hint="Patient Directory · Plexus IQ · Imaging Central · Ancillary Docs · Billing · Engagement · Scheduling · Document Library · Patient Journey · Team Ops · Mission Control." />
      </div>

      <BackendPendingNotice
        message={INTEGRATION_PHASE_1_SUMMARY}
      />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <QuickJumpTile title="Browse vendor adapters" desc="See which EMR vendor adapters ship today and which are catalogued as future adapters." onClick={() => setActive("vendors")} />
        ...
        <QuickJumpTile title="Writeback Readiness" desc="Writeback is OFF in Phase 1. Read the gating checklist before any write scope is enabled." onClick={() => setActive("writeback")} />
      </div>
    </SectionShell>
  );
}

function VendorsSection({ setActive }: { setActive: (id: SectionId) => void }) {
  return (
    <SectionShell
      title="Vendor Adapters"
      subtitle="The Integration Station is vendor-neutral. Each adapter implements the same sync / mapping / routing / audit contract. Phase 1 ships the eClinicalWorks FHIR R4 adapter; everything else is a placeholder."
    >
      <BackendPendingNotice
        message="Future vendor adapters are catalogued so the framework shape is reviewable. They are disabled in the UI and never trigger network calls."
      />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" data-testid="grid-vendor-adapters">
        {INTEGRATION_VENDOR_ADAPTERS.map((adapter) => {
          const Icon = adapter.Icon;
          const isAvailable = adapter.status === "Available";
          return (
            <Card
              key={adapter.vendor}
              className={`rounded-xl p-5 flex items-start gap-4 ${isAvailable ? "border-indigo-200 bg-indigo-50/30" : "border-slate-200 bg-slate-50/40"}`}
              data-testid={`vendor-card-${adapter.shortLabel.toLowerCase()}`}
            >
              <span className={`inline-flex items-center justify-center w-10 h-10 rounded-xl shrink-0 ${isAvailable ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500"}`}>
                <Icon className="w-5 h-5" />
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <div className="text-sm font-semibold text-slate-900">{adapter.vendor}</div>
                  <Badge variant={isAvailable ? "default" : "secondary"} className="text-[10px]">
                    {isAvailable ? "Available" : "Future vendor adapter"}
                  </Badge>
                </div>
                <p className="text-xs text-slate-600 mt-1 leading-snug">{adapter.description}</p>
                {isAvailable ? (
                  <Button size="sm" variant="outline" className="mt-3" onClick={() => setActive("connection")} data-testid={`vendor-open-${adapter.shortLabel.toLowerCase()}`}>
                    Open adapter <ChevronRight className="w-3 h-3 ml-1" />
                  </Button>
                ) : (
                  <Button size="sm" variant="outline" className="mt-3" disabled data-testid={`vendor-disabled-${adapter.shortLabel.toLowerCase()}`}>
                    Not implemented
                  </Button>
                )}
              </div>
...
function WritebackReadinessSection() {
  return (
    <SectionShell
      title="Writeback Readiness"
      subtitle="Writeback is OFF in Phase 1. Every gating requirement below must be signed off — per vendor and per write scope — before any write action is exposed."
      status={<Badge variant="outline" className="text-[10px]">Writeback disabled</Badge>}
    >
      <BackendPendingNotice
        title="Writeback is disabled"
        message="The Integration Station ships read/import-first. Write scopes require vendor approval per tenant, a clinical safety review per resource, audit + idempotency guarantees, and conflict-handling logic. None of these are wired in Phase 1."
        tone="warning"
      />

      <Card className="rounded-xl border-slate-200 p-5">
        <div className="text-sm font-semibold text-slate-800 mb-3">Gating checklist</div>
        <ul className="divide-y divide-slate-100">
          {WRITEBACK_READINESS_REQUIREMENTS.map((req) => (
            <li key={req.id} className="py-3 flex items-start gap-3" data-testid={`writeback-req-${req.id}`}>
              <span className="inline-flex w-5 h-5 shrink-0 items-center justify-center rounded border border-slate-300 bg-white text-slate-400 mt-0.5">
                <ShieldX className="w-3 h-3" />
              </span>
              <div className="min-w-0">
                <div className="text-sm font-medium text-slate-800">{req.label}</div>
                <div className="text-xs text-slate-500 mt-0.5 leading-snug">{req.detail}</div>
              </div>
              <Badge variant="outline" className="text-[10px] shrink-0">Not satisfied</Badge>
            </li>
          ))}
        </ul>
      </Card>
      ...
      <Card className="rounded-xl border-slate-200 p-5">
        <div className="text-sm font-semibold text-slate-800 mb-3">Future write candidates</div>
        <p className="text-xs text-slate-500 mb-3 leading-snug">
          For reference only — none of these are implemented. Patient.write is intentionally last and requires strict controls + per-field allowlist.
        </p>
        <ul className="space-y-2">
          {WRITEBACK_FUTURE_CANDIDATES.map((candidate) => (
            <li key={candidate.resource} className="flex items-start gap-3 text-sm" data-testid={`writeback-candidate-${candidate.resource.replace(/\W+/g, "-").toLowerCase()}`}>
              <Badge variant="outline" className="text-[10px] mt-0.5">{candidate.resource}</Badge>
              <span className="text-slate-600 text-xs leading-snug">{candidate.note}</span>
            </li>
          ))}
        </ul>
      </Card>
