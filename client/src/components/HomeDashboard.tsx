...
function Pill({ children, tone = "steel" }: { children: ReactNode; tone?: "steel" | "dark" | "blue" | "green" | "amber" | "red" }) {
  ...
  return <span className={`inline-flex items-center rounded-none border px-2 py-1 text-[11px] font-black uppercase tracking-[0.08em] ${tones[tone]}`}>{children}</span>;
}
...
function Metric({ label, value, note }: { label: string; value: string | number; note?: string }) {
  return (
    <div className="rounded-none border border-slate-200 bg-slate-50 px-4 py-3">
      ...
      <div className="mt-2 text-[28px] font-black leading-none tracking-[-0.06em] text-slate-950">{value}</div>
      {note && <div className="mt-1 text-[11px] font-medium text-slate-500">{note}</div>}
    </div>
  );
}
...
function SecondaryTile({ href, icon, label, testId }: { href: string; icon: ReactNode; label: string; testId: string }) {
  return (
    <Link href={href}>
      <Card className="group h-full cursor-pointer rounded-none border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.055)] transition-colors hover:border-slate-400" data-testid={testId}>
        <div className="flex h-[96px] items-center gap-3 px-4">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-none bg-slate-100 text-slate-950">{icon}</div>
          <div className="min-w-0 text-[13px] font-black leading-tight tracking-[-0.02em] text-slate-950">{label}</div>
        </div>
      </Card>
    </Link>
  );
...
function MonthControls({ dashboardWeekOverride, dashboardData, setDashboardWeekOverride }: { dashboardWeekOverride: string | null; dashboardData: ScheduleDashboardResponse | undefined; setDashboardWeekOverride: (v: string | null) => void }) {
  ...
  return (
    <div className="flex items-center rounded-none border border-slate-200 bg-white">
      <button type="button" onClick={() => shift(-1)} className="rounded-none p-2 text-slate-600 hover:bg-slate-100" data-testid="button-dashboard-prev-month" aria-label="Previous month"><ChevronLeft className="h-4 w-4" /></button>
      <span className="w-36 border-x border-slate-200 px-3 text-center text-sm font-black tabular-nums text-slate-900" data-testid="text-dashboard-month-label">
        {dashboardData?.weekStart ? new Date(dashboardData.weekStart + "T00:00:00").toLocaleDateString(undefined, { month: "long", year: "numeric" }) : "—"}
      </span>
      <button type="button" onClick={() => shift(1)} className="rounded-none p-2 text-slate-600 hover:bg-slate-100" data-testid="button-dashboard-next-month" aria-label="Next month"><ChevronRight className="h-4 w-4" /></button>
    </div>
  );
}
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800 bg-[#101115] text-white">
        <div className="flex h-16 items-center px-6 lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-sm font-black tracking-tight">Plexus</div>
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Ancillary Screening Platform</div>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
          <div className="space-y-4">
            <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_420px]">
              <Card className="relative min-h-[340px] overflow-hidden rounded-none border border-slate-800 bg-[linear-gradient(135deg,#0D0E12_0%,#171B26_52%,#2A3D5A_100%)] p-8 text-white shadow-[0_34px_90px_rgba(4,8,16,0.28)]" data-testid="tile-plexus-iq">
                ...
                <div className="relative z-10 flex h-full min-h-[276px] flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-none border border-white/15 bg-white/10"><Sparkles className="h-5 w-5 text-white" strokeWidth={1.75} /></div><span className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-300">Primary System</span></div>
                    <h1 className="mt-8 text-[52px] font-black leading-[0.9] tracking-[-0.08em] text-white sm:text-[70px]" data-testid="text-home-heading">Plexus Clinical</h1>
                    <p className="mt-5 max-w-2xl text-[15px] leading-6 text-slate-300">Qualification engine for Batch Flow, Visit, Outreach, Admin Review, packets, and patient progression.</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Link href="/plexus-iq"><span className="inline-flex rounded-none border border-white bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-950">Open Plexus IQ</span></Link>
                    <button type="button" onClick={() => document.getElementById("global-calendar")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="rounded-none border border-white/20 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-white">Global Calendar</button>
                  </div>
                </div>
              </Card>

              <Card className="rounded-none border border-slate-200 bg-white p-5 shadow-[0_18px_48px_rgba(15,23,42,0.055)]">
                <div className="flex items-start justify-between gap-3"><div><div className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">Today</div><div className="mt-2 text-xl font-black tracking-[-0.04em] text-slate-950">{effectiveSelectedDate ? formatDayHeader(effectiveSelectedDate, today) : "Selected day"}</div></div><Pill tone="green">Live</Pill></div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-2 border-t border-slate-200 pt-4"><div><div className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Patients</div><div className="mt-1 text-xl font-black text-slate-950">{selectedPatientsCount}</div></div><div><div className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Clinic</div><div className="mt-1 truncate text-sm font-black text-slate-950">{activeDashboardClinic?.clinicLabel || "—"}</div></div><div><div className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Scheduler</div><div className="mt-1 truncate text-sm font-black text-slate-950">{activeDashboardClinic?.scheduler?.name || "—"}</div></div></div>
              </Card>
            </section>
            ...
            <Card className="rounded-none border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.055)]" data-testid="tile-live-dashboard-row">
              <div className="border-b border-slate-200 px-5 py-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-lg font-black tracking-[-0.04em] text-slate-950">Live Command Row</div><div className="mt-1 text-xs font-medium text-slate-500">Only current action signals.</div></div><Pill tone="dark">Live</Pill></div></div>
            ...
            <Card id="global-calendar" className="scroll-mt-20 rounded-none border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.055)]" data-testid="tile-calendar-bottom">
              ...
              <div className="grid gap-4 p-5 xl:grid-cols-[minmax(0,1fr)_360px]">
                ...
                <div>{dashboardLoading ? <div className="overflow-x-auto"><div className="min-w-[700px]"><div className="mb-2 grid grid-cols-7">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => <div key={d} className="py-2 text-center text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">{d}</div>)}</div><div className="grid grid-cols-7 gap-1.5">{[...Array(42)].map((_, i) => <div key={i} className="h-20 animate-pulse rounded-none bg-slate-100" />)}</div></div></div> : !activeDashboardClinic ? <div className="rounded-none border border-dashed border-slate-200 bg-slate-50 py-16 text-center text-sm text-slate-400">No schedule data.</div> : <div className="overflow-x-auto"><div className="min-w-[700px]"><div className="mb-2 grid grid-cols-7">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => <div key={d} className="py-2 text-center text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">{d}</div>)}</div><div className="grid grid-cols-7 gap-1.5">{activeDashboardClinic.monthCells.map((cell) => { const isToday = cell.isoDate === dashboardData?.today; const isSelected = cell.isoDate === effectiveSelectedDate; const isCurrentMonth = cell.isoDate.slice(0, 7) === displayMonth; const dayNum = parseInt(cell.isoDate.split("-")[2], 10); const previewPatients = (cell.patients ?? []).slice(0, 1); const moreCount = Math.max(0, (cell.patients?.length ?? cell.patientCount) - previewPatients.length); const baseStyle = isToday ? "border-slate-950 bg-slate-950 text-white" : isSelected ? "border-blue-800 bg-white text-slate-950 ring-2 ring-blue-800 ring-inset" : isCurrentMonth ? "border-slate-200 bg-slate-50 text-slate-950 hover:border-slate-400 hover:bg-white" : "border-transparent bg-slate-50/50 text-slate-300 hover:bg-slate-100"; return <button type="button" key={cell.isoDate} onClick={() => setSelectedDate(cell.isoDate)} className={`flex min-h-[84px] cursor-pointer flex-col rounded-none border p-2 text-left transition-colors ${baseStyle}`} data-testid={`dashboard-month-cell-${cell.isoDate}`} aria-pressed={isSelected}><div className="mb-1.5 flex items-center justify-between gap-1"><span className="text-sm font-black tabular-nums">{dayNum}</span>{cell.patientCount > 0 && <span className={`rounded-none px-1.5 py-0.5 text-[10px] font-black tabular-nums ${isToday ? "bg-white/10 text-white" : "bg-white text-blue-800"}`}>{cell.patientCount}</span>}</div>{previewPatients.length > 0 && <div className="overflow-hidden">{previewPatients.map((p) => <span key={p.id} className={`block truncate text-[11px] leading-tight ${isToday ? "text-white/85" : isCurrentMonth ? "text-slate-700" : "text-slate-400"}`} data-testid={`text-cell-patient-${p.id}`}>{p.time && <span className="mr-1 tabular-nums opacity-70">{formatTime12(p.time).replace(/ (AM|PM)$/i, "")}</span>}{firstName(p.name) || "(unnamed)"}</span>)}{moreCount > 0 && <span className={`text-[10px] font-black ${isToday ? "text-white" : "text-blue-800"}`}>+{moreCount}</span>}</div>}</button>; })}</div></div></div>}</div>
                <aside className="rounded-none border border-slate-200 bg-slate-50 p-4" data-testid="panel-day-detail"><div className="mb-4"><div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Selected Day</div><h3 className="mt-2 text-xl font-black tracking-[-0.04em] text-slate-950" data-testid="text-day-detail-header">{effectiveSelectedDate ? formatDayHeader(effectiveSelectedDate, today) : "Selected Day"}</h3>{activeDashboardClinic && <div className="mt-1 text-xs font-medium text-slate-500">{activeDashboardClinic.clinicLabel}{activeDashboardClinic.scheduler && <span className="ml-1.5 text-slate-400">· {activeDashboardClinic.scheduler.name}</span>}</div>}</div><div className="mb-4 grid grid-cols-2 gap-2"><Metric label="Patients" value={selectedPatientsCount} /><Metric label="Ancillary" value={selectedClinicAncillaryCount} /></div>{Object.keys(selectedDayAncillaryBreakdown).length > 0 && <div className="mb-4 flex flex-wrap gap-1.5" data-testid="day-ancillary-breakdown">{Object.entries(selectedDayAncillaryBreakdown).map(([test, n]) => <Pill key={test}>{test} ×{n}</Pill>)}</div>}{selectedDayPatients.length === 0 ? <div className="rounded-none border border-dashed border-slate-200 bg-white py-10 text-center text-sm text-slate-400" data-testid="text-day-empty">No patients scheduled.</div> : <div className="space-y-2" data-testid="list-day-patients">{selectedDayPatients.slice(0, 6).map((p) => <button type="button" key={p.id} onClick={() => onOpenSchedule(p.batchId)} className="grid w-full grid-cols-[68px_1fr] items-center gap-3 rounded-none border border-slate-200 bg-white px-3 py-2.5 text-left transition-colors hover:border-slate-400" data-testid={`button-day-patient-${p.id}`}><span className="text-xs font-black tabular-nums text-blue-800">{formatTime12(p.time) || "—"}</span><span className="min-w-0"><span className="block truncate text-sm font-black text-slate-900" data-testid={`text-day-patient-name-${p.id}`}>{p.name || "(unnamed)"}</span>{p.ancillaries.length > 0 && <span className="mt-1 flex flex-wrap gap-1">{p.ancillaries.slice(0, 2).map((a, i) => <span key={`${p.id}-${a}-${i}`} className="rounded-none bg-slate-50 px-1.5 py-0.5 text-[10px] font-black text-emerald-700 ring-1 ring-emerald-100">{a}</span>)}</span>}</span></button>)}</div>}</aside>
              </div>
            </Card>
          </div>
