...
export type ScheduleDashboardResponse = { today: string; weekStart: string; previousWeekStart: string; nextWeekStart: string; clinicTabs: ClinicTab[] };
...
function countAncillaryLike(breakdown: Record<string, number>, patterns: string[]) {
  return Object.entries(breakdown).reduce((sum, [name, count]) => patterns.some((pattern) => name.toLowerCase().includes(pattern)) ? sum + count : sum, 0);
}
  ...
  return <div className="flex h-full flex-col bg-[#EEF1F6]">
    ...
    <main className="flex-1 overflow-auto"><div className="mx-auto max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8"><div className="space-y-4">
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_420px]">
        <Card className="relative min-h-[340px] overflow-hidden rounded-none border border-slate-800 bg-[linear-gradient(135deg,#0D0E12_0%,#171B26_52%,#2A3D5A_100%)] p-8 text-white shadow-[0_34px_90px_rgba(4,8,16,0.28)]" data-testid="tile-plexus-iq"><div className="pointer-events-none absolute inset-0 opacity-60" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,0.035) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.035) 1px, transparent 1px), radial-gradient(1px 1px at 12% 22%, rgba(255,255,255,0.85) 50%, transparent 51%), radial-gradient(1px 1px at 74% 18%, rgba(255,255,255,0.75) 50%, transparent 51%), radial-gradient(1px 1px at 86% 62%, rgba(255,255,255,0.7) 50%, transparent 51%)", backgroundSize: "56px 56px, 56px 56px, auto, auto, auto" }} aria-hidden="true" /><div className="relative z-10 flex h-full min-h-[276px] flex-col justify-between"><div><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-none border border-white/15 bg-white/10"><Sparkles className="h-5 w-5 text-white" strokeWidth={1.75} /></div><span className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-300">Primary System</span></div><h1 className="mt-8 text-[52px] font-black leading-[0.9] tracking-[-0.08em] text-white sm:text-[70px]" data-testid="text-home-heading">Plexus Clinical</h1><p className="mt-5 max-w-2xl text-[15px] leading-6 text-slate-300">Qualification engine for Batch Flow, Visit, Outreach, Admin Review, packets, and patient progression.</p></div><div className="flex flex-wrap gap-2"><Link href="/plexus-iq"><span className="inline-flex rounded-none border border-white bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-950">Open Plexus IQ</span></Link><button type="button" onClick={() => document.getElementById("global-calendar")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="rounded-none border border-white/20 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-white">Global Calendar</button></div></div></Card>
      ...
      </div>{batches.length > 0 && <div className="mt-8"><Button variant="outline" size="sm" onClick={onOpenSidebar} className="gap-2 rounded-none text-sm" data-testid="button-view-history"><Clock className="h-4 w-4" />Schedule History ({batches.length})</Button></div>}</div></main>
  </div>;
}
