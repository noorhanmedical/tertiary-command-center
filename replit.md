...
**Clinical Intelligence & Governance (server-backed)**: The AI Logic knowledge layer (learning items, governance rules + version history, evidence decisions, audit entries) lives in PostgreSQL — tables `ci_learning_items`, `ci_rules`, `ci_rule_versions`, `ci_evidence_records`, `ci_audit_entries` (`shared/schema/clinicalIntelligence.ts`, text ids like `learn_…`/`rule_…`, ISO-text timestamps). API at `/api/clinical-intelligence/*` (`server/routes/clinicalIntelligence.ts` → `server/repositories/clinicalIntelligence.repo.ts`); rule updates use `FOR UPDATE` + version bump into `ci_rule_versions`; evidence dedupes by patient+label+sourceType while merging forward `usedInRuleIds`/`assignedAncillary`. The 4 example rules are seeded server-side on first boot (empty table only). The client store (`client/src/lib/clinicalIntelligence/store.ts`) keeps its old mutation names but is now async over the API with a single react-query key; legacy browser localStorage data (`plexusIq.clinicalIntelligence.v1`) is one-time imported (insert-only, seed copies skipped) then the key is cleared.
...
**Patient SMS (Task #648)**: Real two-way patient texting via a gated Twilio adapter (`server/integrations/twilioSms.ts` — Replit connector first, then `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_PHONE_NUMBER` env vars). Messages persist in `patient_sms_messages` (migration 0045). API at `/api/portal/patient-messages/*` (status, threads, thread, patients picker, send) plus a public signature-validated inbound webhook `POST /api/sms/twilio/inbound` (exempted from the global `/api` session gate). Sends record "sent" only after Twilio accepts; failures persist as "failed". When Twilio isn't connected, the Team Portal tray's Patients tab shows an honest banner with a disabled composer and sends return 503 `SMS_NOT_CONNECTED` — no fabricated messages. Outbound sends journal to the communication log as genuine kind `sms` (the dormant `sms_scaffold` kind remains, still labeled "not sent"). `scripts/qa-phase-2-hardening-sms-dormant.mjs` now guards the enabled-state honesty invariants.
...
**Role-Based Access Control (RBAC)**: The `users` table includes a `role` column supporting four roles: `admin`, `clinician`, `scheduler`, `biller`. A `requireRole(...roles)` middleware factory enforces access on sensitive server routes (e.g., `DELETE /api/screening-batches/:id` is admin-only; `DELETE /api/billing-records/:id` requires admin or biller). The `GlobalNav` filters navigation items by the logged-in user's role and displays the role label beneath the username. Role is stored in the session and returned by `GET /api/auth/me`. Admins can create users with a specific role via `POST /api/users` and update existing user roles via `PATCH /api/users/:id/role`. The Clinical Intelligence governance page (`/clinical-intelligence`) is role-gated client-side via `client/src/lib/clinicalIntelligence/permissions.ts`: rules in `pending_physician_review` can only be approved/returned by clinician-role users, `pending_compliance_review` only by admins, and all other governance actions (rule creation, learning-item review) require admin or clinician; schedulers/billers see everything read-only. All governance audit/version-history entries record the logged-in user's real username (no hardcoded actor labels).
...
**Team Portal workspace persistence**: Team Portal sticky notes / playground widgets persist per user in the `portal_widgets` table (`/api/portal/widgets`), and workspace settings (default tray tab, sticky-note visibility, pinned panels, playground layout, calendar behavior) persist per user as a jsonb row in the `workspace_prefs` table (`GET`/`PUT /api/portal/workspace-prefs`). Both client hooks hydrate fail-closed — writes are blocked until the initial GET succeeds — and the Settings dialog now says preferences are saved automatically (the old "session only" notice is gone).
