...
| `client/src/components/outreach/TriClinicCalendar.tsx` <br/> `client/src/components/outreach/ExpandedSectionView.tsx` <br/> `client/src/pages/outreach-scheduler-portal.tsx` <br/> *(host: Patient Care Specialist Workspace, `/patient-care-specialist-portal`. Right-panel mode tabs are hydrated from canonical sources — Call List from `/api/scheduler-portal/cases`, Ancillary Schedule from `/api/technician-liaison/ancillary-schedule`. The page-level TriClinicCalendar surface itself is still pending migration to `UniversalCalendar profileId="patientCareSpecialist"`. Note: Outreach / Engagement Center is its own separate surface at `/engagement-center` and is not part of this workspace.)* | `patientCareSpecialist`     | partial — right panel hydrated; full-page calendar pending |
| `client/src/components/clinic-calendar.tsx` (MiniCalendar / SlotGrid) <br/> `client/src/components/portal/PortalShell.tsx` (MonthlyMiniCalendar) <br/> *(host: Ancillary Care Specialist Workspace, `/ancillary-care-specialist-portal`. Right-panel mode tabs are hydrated from canonical sources — Clinic Schedule, Ancillary Schedule, Call List. Procedure-side actions remain ACS-only. Full-page mini-calendar surfaces still pending migration to `UniversalCalendar profileId="technician"`.)* | `technician`                | partial — right panel hydrated; full-page calendar pending |
