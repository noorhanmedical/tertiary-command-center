...
| `client/src/components/outreach/TriClinicCalendar.tsx` <br/> `client/src/components/outreach/ExpandedSectionView.tsx` <br/> `client/src/pages/outreach-scheduler-portal.tsx` <br/> *(host: Patient Care Specialist Workspace, `/patient-care-specialist-portal`. Right-panel mode tabs hydrated from canonical sources. Per-user scoping via Team Member Profile settings is now in place — assigned facilities, default facility, default mode, and capability flags are honored from `admin_settings`. Full-page TriClinicCalendar surface still pending migration to `UniversalCalendar profileId="patientCareSpecialist"`. Note: Outreach / Engagement Center is its own separate surface at `/engagement-center` and is not part of this workspace.)* | `patientCareSpecialist`     | partial — right panel hydrated + profile scope; full-page calendar pending |
| `client/src/components/clinic-calendar.tsx` (MiniCalendar / SlotGrid) <br/> `client/src/components/portal/PortalShell.tsx` (MonthlyMiniCalendar) <br/> *(host: Ancillary Care Specialist Workspace, `/ancillary-care-specialist-portal`. Right-panel mode tabs hydrated from canonical sources. Per-user scoping via Team Member Profile settings is now in place. Procedure-side actions remain ACS-only and now also require the profile capability bit. Full-page mini-calendar surfaces still pending migration to `UniversalCalendar profileId="technician"`.)* | `technician`                | partial — right panel hydrated + profile scope; full-page calendar pending |
