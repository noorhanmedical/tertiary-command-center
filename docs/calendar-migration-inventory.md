...
| `client/src/components/outreach/TriClinicCalendar.tsx` <br/> `client/src/components/outreach/ExpandedSectionView.tsx` <br/> `client/src/pages/outreach-scheduler-portal.tsx` <br/> *(host: Patient Care Specialist Workspace, `/patient-care-specialist-portal`. Right-panel mode tabs and capabilities are now fully **profile-driven** — workspace name no longer hardcodes behavior. Full-page TriClinicCalendar surface still pending migration to `UniversalCalendar profileId="patientCareSpecialist"`. Note: Outreach / Engagement Center is its own separate surface at `/engagement-center` and is not part of this workspace.)* | `patientCareSpecialist`     | partial — right panel profile-driven; full-page calendar pending |
| `client/src/components/clinic-calendar.tsx` (MiniCalendar / SlotGrid) <br/> `client/src/components/portal/PortalShell.tsx` (MonthlyMiniCalendar) <br/> *(host: Ancillary Care Specialist Workspace, `/ancillary-care-specialist-portal`. Right-panel mode tabs and capabilities are now fully **profile-driven**. Procedure-side actions (Procedure Complete, consent/screening primary ownership, upload procedure report) require the matching capability bit in the user's Team Member Profile; workspace name alone is no longer a gate. Full-page mini-calendar surfaces still pending migration to `UniversalCalendar profileId="technician"`.)* | `technician`                | partial — right panel profile-driven; full-page calendar pending |
