...
| `sync_billing_document_reference` | (verify enqueue in Step 12) | `billingRetryHandlers` (`BILLING_RETRY_ACTIONS`) | clinic+BD reference | current BD reference durable (`supersededAt IS NULL`, exact source) | retained | dedup | verify wiring + resolution truth in **K7/K10** |
